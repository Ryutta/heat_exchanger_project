...
```{r}
df_final_u <- left_join(df_outcome_u, df_Stuck, by = "PeriodType") |>
  filter(!is.na(NoS)) |>
...
```{r}
df_final_jacketp <- left_join(df_outcome_jacketp, df_Stuck, by = "PeriodType") |>
  filter(!is.na(NoS)) |>
  mutate(Time_Category = if_else(Reference_Date < as.POSIXct("2021-10-01"), "Before Sep 2021", "After Sep 2021"))
library(DT)
...
```{r}
df_final_pumpp <- left_join(df_outcome_pumpp, df_Stuck, by = "PeriodType") |>
  filter(!is.na(NoS)) |>
  mutate(Time_Category = if_else(Reference_Date < as.POSIXct("2021-10-01"), "Before Sep 2021", "After Sep 2021"))
library(DT)
...
```{r}
df_final_u |>
  ggplot(aes(x = UA, y = NoS, color = Time_Category)) +
  geom_point() +
  ...
  geom_text(aes(label = PeriodType), 
            ...
            size = 3.5,    # 文字サイズ
            check_overlap = FALSE,
            show.legend = FALSE) + # すべて表示するためにFALSEに設定
  labs(x = "UA", y = "NoS") +
  theme_bw() +
  theme(
...
```{r}
df_final_jacketp |>
  ggplot(aes(x = Max_jacketp, y = NoS, color = Time_Category)) +
  geom_point() +
  geom_smooth(method = "lm", se = TRUE) +
  stat_cor(method = "pearson", label.x.npc = "left", label.y.npc = "top") +
  # 全データにラベルを表示
...
```{r}
df_final_pumpp |>
  ggplot(aes(x = Max_P, y = NoS, color = Time_Category)) +
  geom_point() +
  geom_smooth(method = "lm", se = TRUE) +
  stat_cor(method = "pearson", label.x.npc = "left", label.y.npc = "top") +
  # 全データにラベルを表示

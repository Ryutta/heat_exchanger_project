---
title: "M1E-4410 性能評価およびチューブ閉塞分析レポート"
author: "Ryuta"
date: "`r Sys.Date()`"
...
M1粗MAA分離塔リボイラー（M1E-4410）の詰まり本数と各運転パラメータ（U値、ジャケット圧、ポンプ圧）の相関関係を分析し、詰まりの発生要因を特定することを目的とする。
...
tags <- c("M1FC4403.PV", "M1TI4405.PV", "M1PI4412.PV", "M1PI4407.PV", "M1PI4411.PV")
...
if (nrow(all_data) > 0) {
  ...
  if (all(cols_needed %in% names(df_wide))) {
    ...
    df_calc <- df_wide |>
      rename(
        steam_flow = `M1FC4403.PV`,
        t1 = `M1TI4405.PV`,
        jacket_P = `M1PI4412.PV`,
        pump_inlet_raw = `M1PI4407.PV`,
        pump_outlet_raw = `M1PI4411.PV`
      ) |>
      filter(steam_flow > 0.5, t1 > 50, pump_outlet_raw > -50, pump_inlet_raw > 0) |>
      mutate(
        # Constants
        steam_latent_heat = 2257.05,
        density_liquid = 982, # 比重 0.982 (P-4411仕様図面より)
        
        # Step 1: Heat q
        q = steam_flow * steam_latent_heat * 1000 / 3600 / 1000,
        
        # Step 2: Pump Flow (P-4411 500SCQ-EX 性能曲線より)
        # pump_inlet (kPa)
        pump_inlet = (pump_inlet_raw / 760 * 101.325) + (density_liquid * 9.81 * (1.4 + 6.125) / 1000),
        
        # pump_outlet (kPa abs) = gauge + 101.325
        pump_outlet_abs = pump_outlet_raw + 101.325,
        
        # lifting_height (m)
        lifting_height = (pump_outlet_abs - pump_inlet) * 1000 / 9.81 / density_liquid,
        
        # pump_flow (t/h) : P-4411 性能曲線二次近似
        x = lifting_height,
        pump_flow = (-0.6868 * x^2 + 4.0434 * x + 42.5505) * 60 * density_liquid / 1000,
      ...
      filter(U > 100, U < 3000, delta_tlm > 0.5)
...
df_Stuck <- read.csv("data/NoS_M1E_4410.csv") |>
  rename(`b/e` = b_e) |>
...
予測式との乖離幅が最も大きいプロットについて、全体本数（1064本）に対する割合を以下に示す。
